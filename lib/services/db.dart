...
class Collection<T> {
  ...
  Future<void> addData(Map data) {
    DocumentReference docRef = ref.document();
    return docRef.setData(Map<String, dynamic>.from(data), merge: true);
  }
}
...
class FilteredCollection<T> {
  ...
  FilteredCollection({this.path, this.filter}) {
    ref = _db.collection(path);
  }
  ...
  Future<List<T>> getData() async {
    ...
    var snapshots = await ref
        .where(
          filter['field'],
          isEqualTo: filter['value'] ?? user.uid,
        )
        // .orderBy(filter['orderBy'] ?? '', descending: true)
  ...
  Future<void> upsertOne(Map data) async {
    Document<T> ref = Document(path: '$path/${data['id']}');
